...
import { HttpResult, HttpStatus } from '@joingo.vip/http';
import { AxiosResponse } from 'axios';
import { type RyVoid } from './metadata-schemas.mjs';
...
export class Ryu {
  ...
  static hasData(result?: HttpResult<AxiosResponse>): boolean {
    return !console.debugIf(
      Object.isNull(result.rawResponse?.data),
      `[DEBUG] - <ryu.mts: 29ab75>: 若依的 HTTP 服务并未返回任何有效的数据结果。`
    );
  }
  ...
  static isUnauthorized(result: HttpResult<AxiosResponse>): boolean {
    if (result.hasErrors) {
      if (
        console.errorIf(
          result.error.statusCode && result.error.statusCode === HttpStatus.ClientErrorResponse.Unauthorized,
          `[ERROR] - <ryu.mts: eb57a4>: 当前请求的若依 HTTP 服务未授权使用。`
        )
      ) {
        return true;
      }
    ...
    if (Ryu.hasData(result)) {
      const _void = result.rawResponse.data as RyVoid;
      if (
        console.errorIf(
          _void.code && _void.code === HttpStatus.ClientErrorResponse.Unauthorized,
          `[ERROR] - <ryu.mts: eb57a4>: 当前请求的若依 HTTP 服务未授权使用。`
        )
      ) {
        return true;
  ...
  static hasServerError(result: HttpResult<AxiosResponse>): boolean {
    if (result.hasErrors) {
      if (
        console.errorIf(
          result.error.statusCode && result.error.statusCode === HttpStatus.ServerErrorResponse.InternalServerError,
          `[ERROR] - <ryu.mts: eb57a4>: 当前请求的若依 HTTP 服务抛出了未处理的异常。`
        )
      ) {
        return true;
      }
    ...
    if (Ryu.hasData(result)) {
      ...
      if (
        console.errorIf(
          _void.code && _void.code === HttpStatus.ServerErrorResponse.InternalServerError,
          `[ERROR] - <ryu.mts: eb57a4>: 当前请求的若依 HTTP 服务抛出了未处理的异常。`
        )
      ) {
        return true;
  ...
  static getErrorDescription(result: HttpResult<AxiosResponse>, ifNullOrEmpty: string | sys.Func<string>): string {
    if (Ryu.hasData(result) && !String.isNullOrWhitespace((result.rawResponse?.data as RyVoid)?.msg)) {
      return (result.rawResponse.data as RyVoid).msg;
    }
